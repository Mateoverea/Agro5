...
export function CropManagement() {
  ...
  const addCrop = (crop) => {
    setCrops([{ ...crop, id: crops.length + 1, status: "Seeded" }, ...crops])
  }

  const updateCropStatus = (id, status, field = null, expectedHarvest = null) => {
    setCrops(
      crops.map((crop) =>
        crop.id === id
          ? {
      ...
      <CardContent>
        <Tabs defaultValue="crops" className="space-y-4">
          ...
          <TabsContent value="crops">
            <CropTable
              crops={crops}
              onTransplant={(id, field, expectedHarvest) =>
